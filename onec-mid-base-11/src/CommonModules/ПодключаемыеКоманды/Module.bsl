...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	//+Дмитрий@, 6 июня 2024 г. 9:59:31 Домашнее задание к занятию «Внесение изменений в конфигурацию на поддержке»
	ДополнитьФорму(Форма);
	//-/Дмитрий@, 6 июня 2024 г. 9:59:31
...
//+Дмитрий@, 6 июня 2024 г. 12:25:05 Домашнее задание к занятию «Внесение изменений в конфигурацию на поддержке»
...
Процедура ДобавитьПолеКонтактноеЛицоВГруппаШапкаПрава(Форма)
	    ПолеВода  = Форма.Элементы.Добавить("КонтактноеЛицо",Тип("ПолеФормы"), Форма.Элементы.ГруппаШапкаПраво);
		ПолеВода.Вид = ВидПоляФормы.ПолеВвода;
		ПолеВода.ПутьКДанным = "Объект.Расш1_КонтактноеЛицо";
	
КонецПроцедуры		   
	
Процедура ВставитьПолеКонтактноеЛицоНаФормеПередСуммаДокумента(Форма)
	    ПолеВода  = Форма.Элементы.Вставить("КонтактноеЛицо",Тип("ПолеФормы"),, Форма.Элементы.СуммаДокумента);
		ПолеВода.Вид = ВидПоляФормы.ПолеВвода;
		ПолеВода.ПутьКДанным = "Объект.Расш1_КонтактноеЛицо";
	КонецПроцедуры
//+/Дмитрий@, 6 июня 2024 г. 12:25:05
